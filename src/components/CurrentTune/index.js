...
function mapStateToProps(state) {
  const { width, height, tuneRowid, tabsVisibility, zoom, tuning, title, abcText, rowid, loading } = state.currentTune;
  const { playMode, playbackSpeed } = state.audio;
  return { width, height, tuneRowid, tabsVisibility, zoom, tuning, title, abcText, rowid, loading, playMode, playbackSpeed };
}
