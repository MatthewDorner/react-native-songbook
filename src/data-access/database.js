    ...
    return new Promise((resolve, reject) => {
      ...
      this.db.transaction((txn) => {
        if (queriedBy === Constants.CollectionTypes.COLLECTION) {
          query = `select * from Tunes where Collection = ${collection}`;
        } else if (queriedBy === Constants.CollectionTypes.SETLIST) {
          query = `select * from Tunes where Setlists like "%,${collection},%"`;
        }
  ...
  importTuneBook(tuneBook) {
    ...
    this.db.transaction((txn) => {
      tunes.forEach((tune, i) => {
        ...
        const collection = (i % 3) + 1;
        const setlists = `,${(i % 3) + 4},5,`;
        txn.executeSql(`insert into Tunes (Tune, Title, Rhythm, Key, Collection, Setlists) VALUES ("${tune}", "${title}", "${rhythm}", "${key}", "${collection}", "${setlists}")`, [], (tx, res) => {
          // res
        });
      });
    }, (error) => {
